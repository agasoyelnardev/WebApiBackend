...
using WebApi.Application.Features.LiveStreams.Commands.DeleteLiveStreamMessage;
...
using WebApi.Application.Features.LiveStreams.Commands.UpdateLiveStreamMessage;
...
public class LiveStreamsController : ControllerBase
{
    ...
    public async Task<IActionResult> UpdateMessage(Guid id, [FromBody] UpdateLiveStreamMessageCommand command, CancellationToken cancellationToken)
    {
        command.Id = id;
        await _mediator.Send(command, cancellationToken);
        return NoContent();
    ...
    {
        await _mediator.Send(new DeleteLiveStreamMessageCommand(id), cancellationToken);
        return NoContent();
    }
}