...
public class ImportBookFromGoogleBooksCommandHandler : IRequestHandler<ImportBookFromGoogleBooksCommand, Guid>
{
    ...
    public async Task<Guid> Handle(ImportBookFromGoogleBooksCommand request, CancellationToken cancellationToken)
    {
        ...
        var cleanDescription = StripHtml(details.Description);
        ...
        var book = new Book
        {
            ...
            Description = string.IsNullOrWhiteSpace(cleanDescription)
                ? "Google Books-dan idxal edilib. Tam mətn üçün admin panelindən PDF yükləyin."
                : cleanDescription,
            Cover = details.Cover,
            Rating = 0,
            Language = "en",
    ...
    private static string StripHtml(string? html)
    {
        ...
        if (decoded.Length <= 1200)
            return decoded;

        return decoded[..1200].Trim() + "…";
    }
}