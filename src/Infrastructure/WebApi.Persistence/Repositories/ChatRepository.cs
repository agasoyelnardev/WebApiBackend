...
public class ChatRepository : IChatRepository
{
    ...
    public async Task IncrementViewerCountAsync(Guid roomId)
    {
        var room = await _context.StreamRooms.FirstOrDefaultAsync(r => r.Id == roomId);
        if (room is not null)
        {
            room.ViewerCount++;
    ...
    public async Task DecrementViewerCountAsync(Guid roomId)
    {
        var room = await _context.StreamRooms.FirstOrDefaultAsync(r => r.Id == roomId);
    ...
    public async Task<bool> HasActiveRoomByUserAsync(string userId)
        => await _context.StreamRooms
            .AnyAsync(r => r.CreatedByUserId == userId && r.IsLive);
