...
public class RoomPresenceService : IRoomPresenceService
{
    ...
    public int GetUserCount(string roomId)
    {
        ...
        // Otaqdakı unikal istifadəçilərin sayını qaytarır
        return room.Values.Distinct().Count();
    }

    public IReadOnlyList<string> GetDistinctParticipantIds(string roomId)
    {
        if (!Rooms.TryGetValue(roomId, out var room))
            return Array.Empty<string>();

        return room.Values.Distinct().ToList();
    }
}